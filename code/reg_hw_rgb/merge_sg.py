...
if __name__ == '__main__':
    ...
    ############# Change your data path or out path here !!! #########

    data_path = 'data_hw_rgb'
    out_path = 'result_hw_rgb'
    # data_path = 'flir'
    # out_path = 'result2'
    ...
    matching2 = Matching(config2).eval().to(device)
    ...
    for file in fpaths:
        ...
        # print(mim_img1)
        # print(result1)
        mim_path1 = out_dir + '/' + out_path + '/mimout/MIMout/IFMIM/'
        mim_path2 = out_dir + '/' + out_path + '/mimout/MIMout/RGBMIM/'
        ...
        # Save image files
        gan_dir = out_dir + '/' + out_path + '/ganout/GAN_Fake_RGB/'
        ...
        num_base, mi_base, r_base = merge_sg(mode_path1, out_path, im1, im2, device, W, H, psd_img_1, psd_img_2,
                                              out_dir, im_file, matching)
        num_mim, mi_rift, r_rift = merge_sg(mode_path2, out_path, mim_path1, mim_path2, device, W, H, psd_img_1,
                                             psd_img_2, out_dir, im_file, matching2)
        num_gan, mi_gan, r_gan = merge_sg(mode_path3, out_path, gan_path, im2, device, W, H, psd_img_1, psd_img_2,
                                           out_dir, im_file, matching)
        # num_orift, mi_orift, r_orift = merge_sift(mode_path4, out_path, mim_path1, mim_path2, device, W, H, psd_img_1,
        #                                          psd_img_2, out_dir, im_file, matching)
    ...
    np.save(data_stat_dir + '/num_match_base.npy', num_match_base)
    np.save(data_stat_dir + '/num_match_rift.npy', num_match_rift)
    np.save(data_stat_dir + '/num_match_gan.npy', num_match_gan)
    # np.save(data_stat_dir + '/num_match_ormim.npy', num_match_ormim)

    np.save(data_stat_dir + '/MI_base.npy', MI_base)
    np.save(data_stat_dir + '/MI_rift.npy', MI_rift)
    np.save(data_stat_dir + '/MI_gan.npy', MI_gan)
    # np.save(data_stat_dir + '/MI_ormim.npy', MI_ormim)

    np.save(data_stat_dir + '/rate_base.npy', rate_base)
    np.save(data_stat_dir + '/rate_rift.npy', rate_rift)
    np.save(data_stat_dir + '/rate_gan.npy', rate_gan)
    # np.save(data_stat_dir + '/rate_ormim.npy', rate_ormim)

    print(MI_base.mean(), MI_rift.mean(), MI_gan.mean())
    print(rate_base.mean(), rate_rift.mean(), rate_gan.mean())
    print(num_match_base.mean(), num_match_rift.mean(), num_match_gan.mean())
